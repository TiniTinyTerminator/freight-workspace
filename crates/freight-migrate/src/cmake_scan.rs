//! Scan a CMake project for dependencies: `find_package` names,
//! `FetchContent_Declare`, and vendored `add_subdirectory` targets; plus the
//! registry lookup used to fill in versions. Moved out of freight core.

use std::path::{Path, PathBuf};

use freight::resolve::cmake::CMAKE_SYSTEM_PKGS;
...
pub struct ConfiguredRegistries {
    ...
    system: Option<freight::registry::DirectoryRegistry>,
    /// Network registries, in priority order.
    repos: Vec<Box<dyn freight::registry::PackageRepo>>,
    reachable: std::cell::Cell<bool>,
}

impl ConfiguredRegistries {
    pub fn new(config: &freight::toolchain::cache::GlobalConfig) -> Self {
        Self {
            system: freight::registry::DirectoryRegistry::system(),
            repos: freight::registry::repos::registries_in_order(config),
...
impl RegistryResolver for ConfiguredRegistries {
    fn lookup(&self, freight_name: &str) -> Option<RegistryHit> {
        use freight::registry::PackageRepo;
